...
- **Topbar override.** `_includes/topbar.html` shadows the gem's include. Differences from upstream: breadcrumb is suppressed on the landing page (anything where `page.layout` is `home`/`projects` or `page.url == '/'`); the topbar-title also treats `projects` like `home` (shows the site title rather than the layout name); the search input is rendered inline at the left and styled via `.topbar-search` rather than Chirpy's click-to-expand flow; a `.topbar-mode-toggle` button is added that proxies clicks to the (CSS-hidden) sidebar `#mode-toggle` so Chirpy's theme-switching JS keeps working. When updating Chirpy, re-diff this file against the upstream `_includes/topbar.html` for any new conditional branches.
- **SCSS entry point.** `assets/css/jekyll-theme-chirpy.scss` mirrors the gem's entry exactly (`@use 'main'` + production `.bundle` branch). Custom rules live below the `/* append your custom style below */` comment. **Do not use `@import` here** — Chirpy ≥ 7.5 dropped it; the file would still compile but no theme styles would load, breaking the entire site.
