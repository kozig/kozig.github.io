---
layout: post
title: Vim Layer Everywhere with AutoHotkey v2
...
[Vim Layer Everywhere](https://github.com/kozig/Vim-Layer-Everywhere) is an AutoHotkey v2 script that turns CapsLock into a toggle for a Vim-inspired keyboard layer — so you get `hjkl` navigation and other Vim-style bindings in any application, not just your editor.
...
- `Escape` — closes the active window instead of its default behavior

The script uses AHK v2 syntax and runs silently in the system tray. CapsLock's original behavior (toggling caps) is suppressed entirely — if you need caps, use `Shift`.
